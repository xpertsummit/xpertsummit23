...
data "template_file" "student_server_user_data" {
  ...
  vars = {
    ...
    docker_env           = "-e SWAGGER_HOST=http://${local.tags["Owner"]}.${local.lab_dns_name} -e SWAGGER_BASE_PATH=/api -e SWAGGER_URL=http://${local.tags["Owner"]}.${local.lab_dns_name}"
    // docker_env        = "-e SWAGGER_HOST=http://${module.student_fgt.fgt_eip_public} -e SWAGGER_BASE_PATH=/api -e SWAGGER_URL=http://${module.student_fgt.fgt_eip_public}"
  }
}